# DiceSpell `Run Table` Oath Banner Archive File Map 요약

## 3줄 요약
- 새 문서 `docs/dicespell_game_flow_token_oath_archive_file_map_packet_kr.md`는 oath live archive를 열 때 **template / stub / manifest example / source packet이 각각 어떤 실제 파일 경로로 이어지고 누가 언제 채워야 하는지**를 한 장으로 고정한다.
- 목적은 새 ending 화면 설계를 더하는 것이 아니라, 이미 충분한 oath 문서 세트를 **실제 stage 폴더 기준 file routing + owner fill order**로 다시 압축하는 것이다.
- 이제 oath는 `문서를 읽었는데 실제로 어떤 파일을 먼저 만들어야 하지?`를 감으로 처리하지 않고, `live archive root 하나 + template/stub/example 역할 분리 + programmer → UI/아트 → QA → PM` 순서로만 닫는다.

## 한 줄 목표
oath 첫 실전 archive를 `문서 묶음`에서 끝내지 않고, **실제 파일 경로와 작성 순서가 정리된 handoff-ready archive workflow**로 만든다.
...
- oath stage packet / live kickoff / semantic anchor
- required artifacts / candidate review
- template / stub / manifest example 파일들

하지만 실제 first live oath archive 직전에는 여전히 이런 질문이 남아 있었다.
...
- `docs/artifacts/run-table-token-delivery/examples/manifest_example_oath_banner.md`
...
2. UI가 `captures/oath-1440-overview.png`, `captures/oath-1440-banner-focus.png`, `notes/density-review.md`를 채운다.
3. 아트가 `assets/starter-asset-list.md`를 채운다.
4. QA가 `notes/handoff-line.md`에서 `archive-ready candidate`를 기록한다.
5. PM이 archive path와 함께 `oath-banner green, queue-closing review-only unlock`을 남긴다.
...
| `notes/hook-manifest.md` | oath root / role / CTA locator와 non-change line |
| `checks/dom-sanity.md` | hook 존재/정합성 점검 |
| `captures/oath-1440-overview.png` | oath banner-first + stage first-glance 증거 |
| `captures/oath-1440-banner-focus.png` | banner / result / carry role 관계 집중 증거 |
| `notes/density-review.md` | 유지·약함·과함 verdict |
| `assets/starter-asset-list.md` | Oath Banner Starter 충분성 verdict |
...
## 즉시 다음 액션
1. 실제 oath turn에서는 source-of-truth map / gap ledger 뒤에 이 file map packet으로 live destination 경로를 먼저 고정한다.
...
## 원본 문서
- source handoff: `./dicespell_game_flow_token_oath_archive_file_map_packet_kr.md`
